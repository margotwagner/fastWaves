...
N_PAIRS=1
SEQ_LEN=$((2 * N_PAIRS + DELAY_STEPS + QUERY_HOLD_STEPS))
...
run_one() {
  local model="$1"
  local seed="$2"
  local run_name="transition_sanity_norm_${model}_p${N_PAIRS}_d${DELAY_STEPS}_q${QUERY_HOLD_STEPS}_seed${seed}"
...
python scripts/summarize_transition_recall.py \
  --runs-dir data/runs \
  --pattern "transition_sanity_norm_*_p${N_PAIRS}_d${DELAY_STEPS}_q${QUERY_HOLD_STEPS}_seed*" \
  --out-run-level data/runs/transition_sanity_norm_run_level.csv \
  --out-summary data/runs/transition_sanity_norm_summary.csv
